...
df["date"] = df[time_col].dt.date  # for overnight flat logic
...
# ==========================
# STATE PER LEVEL (for BREAKOUT + RETEST continuation)
# ==========================
# mode: "idle" or "waiting_retest"
...
def check_entry_signal(i, row, prev_close):
    """
    Continuation logic (breakout + retest):

    Long:
      - prev_close < L and current close > L (break up through level)
      - Later, price retests near L from above -> go long

    Short:
      - prev_close > L and current close < L (break down through level)
      - Later, price retests near L from below -> go short
    """
    global level_state
    ...
    for L in levels:
        ...
        # --- Detect breakout through the level ---
        if mode == "idle":
            # Bullish breakout: close goes from below L to above L
            if (prev_close < L) and (row_close > L) and (row_high >= L):
                state["mode"] = "waiting_retest"
                state["direction"] = "long"

            # Bearish breakout: close goes from above L to below L
            elif (prev_close > L) and (row_close < L) and (row_low <= L):
                state["mode"] = "waiting_retest"
                state["direction"] = "short"

        # --- Retest = actual continuation entry ---
        elif mode == "waiting_retest":
            direction = state["direction"]

            if direction == "long":
                # Retest from above: price dips near L
                ...
                # Retest from below: price pops near L
...
for i in range(1, len(df)):
    ...
    # ==========================
    # FLATTEN OVERNIGHT:
    # if the calendar date changes, close any open trade at prev_close
    # ==========================
    day = row["date"]
    prev_day = prev_row["date"]

    if open_trade is not None and day != prev_day:
        # close at previous bar's close (prev_close)
        if open_trade["direction"] == "long":
            pnl = (float(prev_close) - float(open_trade["entry"])) * open_trade["units"]
        else:  # short
            pnl = (float(open_trade["entry"]) - float(prev_close)) * open_trade["units"]
    ...
    # Manage open trade first (during the same day)
...
# Forced exit at very last candle if still in a trade
if open_trade is not None:
    last_row = df.iloc[-1]
...
if not trades_df.empty:
    ...
    trades_df.to_csv("spx_5to1_trades_continuation_no_overnight.csv", index=False)
    print("\nSaved trades to spx_5to1_trades_continuation_no_overnight.csv")
else:
...
if not trades_df.empty:
    ...
    plt.xlabel("Time")
    plt.ylabel("Equity")
    plt.title("Equity Curve - 5:1 RR Breakout + Retest (No Overnight Holds)")
    plt.tight_layout()
    plt.show()
